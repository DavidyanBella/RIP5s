...
import {T_ArtworksFilters} from "modules/types.ts";
import ArtworksTable from "components/ArtworksTable/ArtworksTable.tsx";
...
const ArtworksPage = () => {
    ...
    const artworks = useAppSelector((state) => state.artworks.artworks)

    const filters = useAppSelector<T_ArtworksFilters>((state) => state.artworks.filters)
    ...
    const applyFilters = async (e) => {
        ...
        const filters:T_ArtworksFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {artworks.length ? <ArtworksTable artworks={artworks}/> : <h3 className="text-center mt-5">Произведения не найдены</h3>}
        </Container>
    )
};
