...
const CharactersListPage = ({characters, setCharacters, isMock, setIsMock}:Props) => {
    ...
    const {character_name} = useAppSelector((state) => state.characters)
    ...
    const fetchCharacters = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/characters/?character_name=${character_name.toLowerCase()}`)
